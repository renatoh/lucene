...
/**
 * This processor verifies if {@link ConfigurationKeys#ANALYZER} is defined in the {@link
 * QueryConfigHandler}. If it is and the analyzer is not <code>null</code>, it looks for every
 * {@link FieldQueryNode} that is not {@link WildcardQueryNode}, {@link FuzzyQueryNode} or {@link
 * RangeQueryNode} contained in the query node tree, then it applies the analyzer to that {@link
 * FieldQueryNode} object. <br>
 * <br>
 * If the analyzer return only one term, the returned term is set to the {@link FieldQueryNode} and
 * it's returned. <br>
 * <br>
 * If the analyzer return more than one term, a {@link TokenizedPhraseQueryNode} or {@link
 * MultiPhraseQueryNode} is created, whether there is one or more terms at the same position, and
 * it's returned. <br>
 * <br>
